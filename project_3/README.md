...
1. You'll collect posts from two subreddits of your choosing. You can use Reddit's API or any other low-code platform of the same. 
...
2. You'll then use NLP to train a classifier on which subreddit (or other sub-sections based on the site you choose to scrape your data from) a given post came from. This is a binary classification problem.
...
To help you get started, we have a primer video on how to use the API: https://www.youtube.com/watch?v=5Y3ZE26Ciuk
...
Pro tip 2: The API will cap you at 1,000 posts for each subreddit (assuming the subreddit has that many posts). The API would also give duplicate posts
...
- Gather and prepare your data using the `requests` or `praw` library.
- **Create and compare at least two models**. One of them must be Naive Bayes and the other can be a classifier of your choice: logistic regression, KNN, SVM, Random Forest, etc.
...
- Presentations will be 20 minutes. You can use inspiration from the [ignite talks](http://www.ignitetalks.io/) framework to design your presentation (its fun!). Learn more about the [ignite style](https://speakingaboutpresenting.com/content/fast-ignite-presentation/) 
...
- Does the student test and evaluate a variety of models to identify a production algorithm (**AT MINIMUM:** Naive Bayes)?
